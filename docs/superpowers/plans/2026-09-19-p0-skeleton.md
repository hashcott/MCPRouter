...
- **Every version in this plan was resolved against the live registry on 2026-09-19 and the load-bearing ones were executed**, not read from docs: `tsc -b` with project references across a pnpm workspace (emits + runs), `vitest@5.0.1` `test.projects` with `--project unit` (filters correctly), and the 8 spikes in §14 of the spec. `uv 0.12.17` is the current release; `@types/node 26.6.1` is current. Re-resolve before starting if more than a few weeks have passed. **Two pins were corrected during execution**: `prettier` and `@types/node` were recorded from a registry `latest` that briefly advertised 3.9.8 / 26.6.2; those tarballs 404 and the real releases are 3.9.7 / 26.6.1. Resolving a version is not the same as fetching it — prefer `npm view <pkg>@<ver> dist.tarball`.
...
- Produces:
  ...
  - `export async function runMigrations(pool: Pool, log: Logger, migrationsDir?: string): Promise<void>` — serialised across replicas by `pg_advisory_lock(MIGRATION_LOCK_ID)`. `migrationsDir` overrides the relative lookup, which only resolves in a dev checkout.
...
```bash
pnpm --filter @mcprouter/server add hono@4.13.8 @hono/node-server@2.1.1 prom-client@15.1.3 pino@10.3.1
```
...
Write `packages/web/package.json` FIRST — `pnpm --filter` cannot resolve a package that has
no manifest yet.

```bash
mkdir -p packages/web/src
# ...write packages/web/package.json (below), then:
pnpm --filter @mcprouter/web add react@19.3.0 react-dom@19.3.0
pnpm --filter @mcprouter/web add -D vite@8.3.0 @vitejs/plugin-react@6.1.1 @types/react@19.1.11 @types/react-dom@19.1.7 typescript@7.0.2
```
...
- [ ] **Step 1: The drift gate needs a script, not a `&&` chain**

`drizzle-kit generate && git diff --exit-code drizzle/` is green in both cases it
exists to catch. Write `scripts/db-check.mjs` instead and point `db:check` at it:
...
//     straight through.
//
// Both are proven by breaking them on purpose; see Step 3.
...
> No CODEOWNERS. The original plan added one so that disabling a gate needed a
> second approval — but this is a single-maintainer open-source repo, there is no
> second approver, and a CODEOWNERS naming a handle nobody reviews for is inert.
> Add it the day a second maintainer exists.
...
jobs:
  verify:
    ...
    steps:
      ...
      - name: build
        run: pnpm build
      # One run, both projects. Coverage measured over unit-only would count the
      # db layer as dead code when it is in fact covered by the integration
      # project, and the gate would fail for a reason that is not true.
      - name: tests (unit + real postgres via testcontainers) + coverage gates
...
# drift gate, case 1: a rename, which makes drizzle-kit crash
...
git checkout drizzle/ 2>/dev/null || true; git clean -fdq drizzle/

# drift gate, case 2: a new column, which writes an UNTRACKED migration
sed -i.bak "s|  value: jsonb('value').notNull(),|  value: jsonb('value').notNull(),\n  note: text('note'),|" packages/core/src/db/schema/index.ts && pnpm db:check; echo "exit=$?"
mv packages/core/src/db/schema/index.ts.bak packages/core/src/db/schema/index.ts
git checkout drizzle/ 2>/dev/null || true; git clean -fdq drizzle/
```
...
This step earned its keep on the first run: with the original `&&` chain both drift
cases exited **0**. Case 1 because drizzle-kit returns success after printing a
stack trace, case 2 because `git diff` does not see untracked files. The lint and
build gates bit correctly (1 and 2).
